...
y = 1
while x < int(number):
    os.system('echo mkdir %s/copydir_%d' %(location, y))
    os.system('echo cp %s %s/copydir_%d/copy%d_of_%s' %(filename, location, y, y, filename))
    os.system('echo ip tuntap add dev tap%d mode tap user $(whoami)' % y)
    os.system('echo ip link set tap%d master br0' % y)
    os.system('echo ip link set dev tap%d up' % y)
    x+=1
    y+=1
    time.sleep(1)
