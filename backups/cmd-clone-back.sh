while getopts ":hc:l:t:" opt; do
  case ${opt} in
    h )
      echo " "
      echo "Usage: cmd-clone.sh -c /dev/sda -l /mnt -t user@192.168.x.x"
      echo " "
...
if [ $(blockdev --getsize64 $clone) -ne $(stat -c %s $location/disk.img) ]; then
    ...
    echo " "
    echo " "
    scp $location/disk.img $transfer:/root/samProject/landingStation/
    if [ $? -eq 0]; 
    then
  	echo " "
        echo "disk.img successfully transfered"
	echo " "
    else 
	echo " "
        echo "disk.img unsuccesfully transfered, Parametres were possibly wrong, try a manual scp"
	echo " " 
    fi
fi
